...
export class Game {
  ...
  stepRace(dt, frozen) {
    ...
    for (const car of this.cars) poseVehicle(car.mesh, this.track, car.state);
    if (this.ghost) {
      ...
      if (g && p.raceLap > 0) {
        ...
        poseVehicle(this.ghost.mesh, this.track, { ...p, s: (p.raceLap - 1) * this.track.length + g.s, lateral: g.lateral, heading: g.heading });
      } else {
  ...
  updateReplay(dt) {
    ...
    if (snap) {
      snap.cars.forEach((pose, i) => {
        ...
        poseVehicle(this.cars[i].mesh, this.track, this.cars[i].state);
      });
